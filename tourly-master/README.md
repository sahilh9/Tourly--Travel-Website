...
This starts an Express server on `http://localhost:3000`, serves the site, and creates `tourly.db` (SQLite) with a `users` table.
...
The forms in `signup.html` and `login.html` are wired via `assets/js/auth.js`.
